...
public class DrawSegment extends DrawTrapezoid {
    ...
    private void setSegmentSide(Blueprint segment, segPosition s) {
        Map<Side, Integer> segSide = new HashMap<>();
        segSide.put(SIDE_X, LEFT);
        segSide.put(SIDE_Y, TOP);
        if(s == T || s == C || s == B) {
            segSide.put(DIR, HORIZONTAL);
        }
        else if(s == LT || s == LB || s == RT || s == RB) {
            segSide.put(DIR, VERTICAL);
        }

        segment.setSide(segSide.get(SIDE_X), segSide.get(SIDE_Y), segSide.get(DIR) );
    }
    // パラメータのリサイズ用
